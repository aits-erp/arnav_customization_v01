...
class DebitNote(PurchaseInvoice):
			...
			if (
				...
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					...
					if item.purchase_receipt:
						...
						# This is done in cases when Debit Note is created before Purchase Receipt
						if (
							for_validate
							and item.expense_account
							and item.expense_account != stock_not_billed_account
							...
							msg += "<br>"
							msg += _(
								"This is done to handle accounting for cases when Purchase Receipt is created after Debit Note"
							)
							frappe.msgprint(msg, title=_("Expense Head Changed"))
	...
	def validate_for_repost(self):
		self.validate_write_off_account()
		self.validate_expense_account()
		validate_docs_for_voucher_types(["Debit Note"])
	...
	def make_supplier_gl_entry(self, gl_entries):
		"""
		Create supplier payable/receivable GL entry for Debit Note.

		Purchase Invoice normally CREDITS supplier (increase payable).
		Debit Note (purchase return) should DEBIT supplier (decrease payable).
		"""

		# compute grand totals (use rounded logic if present)
		grand_total = (
			self.rounded_total if (self.rounding_adjustment and self.rounded_total) else self.grand_total
		)
		base_grand_total = flt(
		...
		if grand_total and not self.is_internal_transfer():
			# determine the party account and currency-aware amounts using get_gl_dict,
			# keeping the sign rules for Debit Note:
			# -> debit = base_grand_total (we decrease supplier liability)
			# -> credit = 0

			# find against_voucher: for return, if update_outstanding_for_self false and is_return with return_against set, use return_against
			against_voucher = self.name
			if self.is_return and self.return_against and not self.update_outstanding_for_self:
				against_voucher = self.return_against

			# currency-aware values: if party_account_currency == company_currency, debit_acc = base_grand_total else debit_acc uses grand_total
			if getattr(self, "party_account_currency", None) == getattr(self, "company_currency", None):
				debit_acc = base_grand_total
				debit_in_trans = grand_total
			else:
				debit_acc = base_grand_total
				debit_in_trans = grand_total

			# Build GL entry for Supplier (DEBIT for Debit Note)
			gl_entries.append(
				self.get_gl_dict(
					{
						"account": self.credit_to or frappe.db.get_value("Party Account", {"party": self.supplier, "party_type": "Supplier", "company": self.company}, "default_account"),
						"party_type": "Supplier",
						"party": self.supplier,
						"due_date": self.due_date,
						"against": self.against_expense_account or None,

						"debit": flt(base_grand_total),
						"credit": 0,

						"debit_in_account_currency": debit_acc,
						"credit_in_account_currency": 0,

						"debit_in_transaction_currency": debit_in_trans,
					...
					getattr(self, "party_account_currency", None),
					item=self,
				)
			)
	...
	def make_provisional_gl_entry(self, gl_entries, item):
		if item.purchase_receipt:
			...
			if pr_item.get("has_provisional_entry"):
				...
				# Intentionally passing Debit Note item to handle partial billing
	...
	def make_gle_for_rounding_adjustment(self, gl_entries):
		...
		if not self.is_internal_transfer() and self.rounding_adjustment and self.base_rounding_adjustment:
			(
				...
				self.company, "Debit Note", self.name, self.use_company_roundoff_cost_center
			)
		...
		if self.bill_no:
			if cint(frappe.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				...
				if pi:
					...
					frappe.throw(
						_("Supplier Invoice No exists in Debit Note {0}").format(
							get_link_to_form("Debit Note", pi)
						)
					)
	...
	def get_pr_details_billed_amt(self):
		# Get billed amount based on purchase receipt item reference (pr_detail) in Debit Note
		...
		if not status:
			if self.docstatus == 2:
				...
				# Check if outstanding amount is 0 due to debit note issued against invoice
					...
					"Debit Note", {"is_return": 1, "return_against": self.name, "docstatus": 1}
				):
...
def make_debit_note(source_name, target_doc=None):
	...
	return make_return_doc("Debit Note", source_name, target_doc)
...
def make_stock_entry(source_name, target_doc=None):
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Debit Note": {"doctype": "Stock Entry", "validation": {"docstatus": ["=", 1]}},
...
def change_release_date(name, release_date=None):
	if frappe.db.exists("Debit Note", name):
		pi = frappe.get_doc("Debit Note", name)
...
def make_inter_company_sales_invoice(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Debit Note", source_name, target_doc)
