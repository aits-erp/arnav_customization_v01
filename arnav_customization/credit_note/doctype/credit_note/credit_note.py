...
class CreditNote(SellingController):\
	...
	def validate_for_repost(self):
		...
		self.validate_account_for_change_amount()
		self.validate_income_account()
		validate_docs_for_voucher_types(["Credit Note"])
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				...
				if self.doctype == "Credit Note" and self.docstatus == 1:
	...
	def on_submit(self):
		...
		if not self.is_return and not self.is_consolidated and self.loyalty_program:
			...
			against_si_doc = frappe.get_doc("Credit Note", self.return_against)
	...
	def check_if_consolidated_invoice(self):
		# since POS Invoice extends Credit Note, we explicitly check if doctype is Credit Note
		if self.doctype == "Credit Note" and self.is_consolidated:
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
					frappe.bold(_("Consolidated Credit Note")),
			...
			self.status_updater.append(
				{
					...
					"extra_cond": """ and exists(select name from `tabCredit Note`
				...
				self.status_updater.append(
					{
						...
						"second_join_field": "so_detail",
						"extra_cond": """ and exists (select name from `tabCredit Note` where name=`tabSales Invoice Item`.parent and update_stock=1 and is_return=1)""",
					}
				)
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if doctype in ["Credit Note", "Sales Order"]:
	...
	if inter_company_reference:
		...
		ref_party = doc.supplier if doctype in ["Credit Note", "Sales Order"] else doc.customer
...
def update_linked_doc(doctype, name, inter_company_reference):
	if doctype in ["Credit Note", "Purchase Invoice"]:
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	if doctype in ["Credit Note", "Purchase Invoice"]:
		ref_doc = "Purchase Invoice" if doctype == "Credit Note" else "Credit Note"
		ref_field = "inter_company_invoice_reference"
	else:
...
def make_maintenance_schedule(source_name, target_doc=None):
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Credit Note": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
			"Sales Invoice Item": {
...
def make_delivery_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Credit Note": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
			"Sales Invoice Item": {
...
def make_sales_return(source_name, target_doc=None):
	...
	return make_return_doc("Credit Note", source_name, target_doc)


def get_inter_company_details(doc, doctype):
	if doctype in ["Credit Note", "Sales Order", "Delivery Note"]:
...
def validate_inter_company_transaction(doc, doctype):
	...
	if not party:
		partytype = "Supplier" if doctype in ["Credit Note", "Sales Order"] else "Customer"
...
def make_inter_company_purchase_invoice(source_name, target_doc=None):
	return make_inter_company_transaction("Credit Note", source_name, target_doc)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Credit Note", "Sales Order"]:
		...
		target_doctype = "Purchase Invoice" if doctype == "Credit Note" else "Purchase Order"
		target_detail_field = "sales_invoice_item" if doctype == "Credit Note" else "sales_order_item"
		source_document_warehouse_field = "target_warehouse"
		target_document_warehouse_field = "from_warehouse"
		received_items = get_received_items(source_name, target_doctype, target_detail_field)
	else:
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Credit Note" if doctype == "Purchase Invoice" else "Sales Order"
...
def create_invoice_discounting(source_name, target_doc=None):
	invoice = frappe.get_doc("Credit Note", source_name)
